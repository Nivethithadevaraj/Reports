...
namespace MovieLensApp.Controller
{
    public class AnalyticsController
    {
        ...
        public void Run()
        {
            ...
            string moviesPath = Path.Combine(projectRoot, "Data", "movies.csv");
            string ratingsPath = Path.Combine(projectRoot, "Data", "ratings.csv");
            ...
            while (!exit)
            {
                ...
                switch (choice)
                {
                    case 1:
                        ...
                        RunAndMeasure(() => generator.GenerateReports(false, Path.Combine(projectRoot, "output", "WithoutThreads")), "Without Threads");
                        break;

                    case 2:
                        view.ShowMessage("Generating reports WITH threads...");
                        RunAndMeasure(() => generator.GenerateReports(true, Path.Combine(projectRoot, "output", "WithThreads")), "With Threads");
                        break;
